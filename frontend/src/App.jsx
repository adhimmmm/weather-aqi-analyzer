...
function App() {
  ...
  const [coords, setCoords] = useState({ lat: -6.2088, lon: 106.8456 }); // Default: Jakarta
    ...
    fetch(`http://localhost:5000/api/weather?lat=${latitude}&lon=${longitude}`)
      ...
      .then((result) => {
        ...
        if (result.status === "success") {
          ...
          throw new Error(result.message || "Terjadi kesalahan pada data server");
        }
        setLoading(false);
      })
      .catch((err) => {
  ...
  function MapClickHandler() {
    useMapEvents({
      click(e) {
        ...
        fetchWeatherAndAir(lat, lng); // Tembak API baru berdasarkan titik klik!
      },
    });
    return null;
  }
    ...
    return (
      <div className="min-h-screen bg-slate-950 text-white flex items-center justify-center p-4">
        <div className="bg-red-500/10 border border-red-500/30 p-8 rounded-2xl text-center max-w-md shadow-2xl backdrop-blur-md">
          <div className="w-16 h-16 bg-red-500/20 text-red-400 rounded-full flex items-center justify-center text-3xl mx-auto mb-4">✕</div>
          <h2 className="text-xl font-bold text-red-400 mb-2">Gagal Memuat Data</h2>
          <p className="text-slate-400 text-sm leading-relaxed">{error}</p>
          <button onClick={() => window.location.reload()} className="mt-6 px-5 py-2 bg-slate-900 border border-slate-800 hover:bg-slate-800 text-white font-medium text-sm rounded-xl transition-all">
            Coba Lagi
          </button>
        </div>
      </div>
        ...
        <div className="text-center md:text-left">
          <div className="flex items-center justify-center md:justify-start gap-2 mb-1">
            ...
            <h1 className="text-2xl md:text-3xl font-black text-white tracking-tight">
              ...
              {data && <span className="text-slate-400 font-semibold">{data.location.country}</span>}
            </h1>
          </div>
          <p className="text-slate-500 text-sm font-medium">
            Klik titik mana saja di peta untuk menganalisis daerah tersebut [cite: 551]
          </p>
        </div>
      ...
      {data && theme && (
        <div className="w-full max-w-5xl space-y-6">
            ...
            <div className="bg-slate-900 border border-slate-800/80 rounded-3xl p-6 shadow-2xl flex flex-col justify-between relative overflow-hidden group hover:border-slate-700 transition-all duration-300">
              ...
              <div>
                <div className="flex items-center justify-between mb-6">
                  <h2 className="text-xs font-bold text-slate-500 tracking-widest uppercase">Cuaca Terkini [cite: 143]</h2>
                  <span className="text-2xl animate-bounce duration-1000">☁️</span>
                </div>
                <div className="my-4">
                  <p className="text-6xl font-black text-white tracking-tighter inline-block relative">
                    {data.weather.temperature_celsius}
                    <span className="text-3xl font-light text-blue-400 absolute -top-2 -right-6">°C</span>
                  </p>
              ...
              <div className="mt-6 pt-4 border-t border-slate-800/60 flex items-center justify-between text-sm">
                <div>
                  <p className="text-xs text-slate-500 font-medium uppercase tracking-wider">Kelembapan [cite: 143]</p>
                  <p className="text-lg font-bold text-blue-400 mt-0.5">{data.weather.humidity_percentage}% [cite: 143]</p>
                </div>
                <div className="text-right">
                  <p className="text-xs text-slate-500 font-medium uppercase tracking-wider">Suhu Sensasi</p>
                  <p className="text-lg font-bold text-slate-200 mt-0.5">{data.weather.temperature_celsius}°C</p>
                </div>
              </div>
            </div>

            {/* UTAMA TENGAH & KANAN: KARTU KUALITAS UDARA */}
            <div className={`md:col-span-2 border rounded-3xl p-6 shadow-2xl transition-all duration-500 flex flex-col justify-between relative overflow-hidden ${theme.bg} ${theme.glow}`}>
              <div>
                <div className="flex items-center justify-between mb-6">
                  <h2 className="text-xs font-bold opacity-60 tracking-widest uppercase">Indeks Kualitas Udara (AQI) [cite: 1, 144]</h2>
                  <span className={`px-3 py-1 rounded-full text-xs font-bold border uppercase tracking-wider ${theme.badge}`}>
                ...
                <div className="flex flex-col sm:flex-row sm:items-end justify-between gap-6 my-2">
                  <div>
                    <p className="text-7xl font-black tracking-tighter">
                      ...
                      <span className="text-xl font-normal opacity-40 ml-2">/ 5</span>
                    </p>
                    <p className="text-sm font-medium opacity-80 mt-2 max-w-sm leading-relaxed">
                      Tingkat polusi udara saat ini berada pada kategori berkadar <span className="font-bold underline">{data.air_quality.aqi_label}</span>. [cite: 144]
                    </p>
                  </div>
                  <div className="p-4 bg-slate-950/40 border border-slate-800/40 rounded-2xl min-w-35 text-center sm:text-right">
                    <p className="text-xs opacity-50 font-medium uppercase tracking-wider mb-0.5">Partikel PM2.5</p>
                    <p className="text-3xl font-black text-white">{data.air_quality.pm2_5}</p>
              ...
              <div className="mt-6 w-full bg-slate-950/50 h-2 rounded-full overflow-hidden p-0.5 border border-slate-800/40">
                <div 
                  className={`h-full rounded-full transition-all duration-1000 ${
                    data.air_quality.aqi_value <= 2 ? 'bg-emerald-400' : data.air_quality.aqi_value === 3 ? 'bg-amber-400' : 'bg-rose-500'
                  }`}
                  style={{ width: `${(data.air_quality.aqi_value / 5) * 100}%` }}
                ></div>
              </div>
            </div>
          </main>
          ...
          <div className="w-full bg-slate-900 border border-slate-800 rounded-3xl p-4 shadow-2xl">
            <h2 className="text-xs font-bold text-slate-400 tracking-widest uppercase mb-3 px-2">Peta Penjelajah Wilayah </h2>
          ...
          <section className="bg-slate-900 border border-slate-800/80 rounded-3xl p-6 md:p-8 shadow-2xl relative overflow-hidden">
            <div className="flex items-center gap-3 mb-4">
              ...
              <h2 className="text-sm font-bold text-slate-400 tracking-widest uppercase">Rekomendasi Aktivitas Sehat [cite: 145]</h2>
            </div>
            ...
            <div className="bg-slate-950 border border-slate-800/60 p-4 md:p-5 rounded-2xl mb-6 shadow-inner flex items-center gap-4">
              <div className={`w-3 h-12 rounded-full shrink-0 ${
                data.recommendations.status_level === 'success' ? 'bg-emerald-500' : data.recommendations.status_level === 'warning' ? 'bg-amber-500' : 'bg-rose-500'
              }`}></div>
              ...
              {data.recommendations.activities.map((activity, index) => (
                ...
                >
                  <div className={`w-8 h-8 rounded-full shrink-0 flex items-center justify-center font-bold text-sm ${
                    activity.allowed ? 'bg-emerald-500/20 text-emerald-400' : 'bg-rose-500/20 text-rose-400'
                  }`}>
                    {activity.allowed ? '✓' : '✕'}
                  </div>
                  <div>
                    <h3 className="font-bold text-white text-base tracking-wide flex items-center gap-2">
                      ...
                      <span className={`text-[10px] px-2 py-0.5 rounded-full font-extrabold tracking-wide uppercase ${
                        activity.allowed ? 'bg-emerald-500/20 text-emerald-300' : 'bg-rose-500/20 text-rose-300'
                      }`}>
                        {activity.allowed ? 'Disarankan' : 'Hindari'}
                      </span>
                    </h3>
                    <p className="text-slate-400 text-sm mt-1 leading-relaxed font-medium">{activity.note}</p>
                  </div>
                </div>
              ))}
            </div>
